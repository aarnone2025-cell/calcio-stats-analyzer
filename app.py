...
def make_coupon_from_manual_odds(evaluated_df, mode="Equilibrata", legs=5):
    if evaluated_df.empty:
        return pd.DataFrame(), {}

    params = {
        "Prudente": {"min_p": 0.62, "min_value": 0.0},
        "Equilibrata": {"min_p": 0.52, "min_value": 2.0},
        "Alta quota": {"min_p": 0.40, "min_value": 4.0},
    }[mode]
    ...
    c = c[
        ...
        & (c["Quota bookmaker"] > 1.0)
        & ((c["Prob. modello %"] / 100.0) >= params["min_p"])
        & (c["Value %"] >= params["min_value"])
    ].copy()

    if c.empty:
        return pd.DataFrame(), {}

    # Score balances model probability and expected value.
    c["_score"] = (
        (c["Prob. modello %"] / 100.0) ** 1.25
        * np.log(c["Quota bookmaker"].clip(lower=1.01))
        + (c["Value %"] / 100.0).clip(lower=-0.2) * 0.75
    )

    # One selection per match.
    c = c.sort_values("_score", ascending=False).drop_duplicates("Partita").head(int(legs)).copy()

    total_odd = float(np.prod(c["Quota bookmaker"])) if not c.empty else np.nan
    joint_p = float(np.prod(c["Prob. modello %"] / 100.0)) if not c.empty else np.nan
...
with st.sidebar:
    ...
    coupon_mode = st.selectbox(
        "Tipo schedina",
        ["Prudente", "Equilibrata", "Alta quota"],
        index=1,
        disabled=not enable_match_model,
    )
...
if enable_match_model and match_prediction_rows:
    ...
    st.caption(
        "Il modello combina risultati recenti, rendimento casa/trasferta, gol fatti/subiti "
        "e un fattore giocatori basato su tiri in porta recenti e continuità del nucleo con più minuti."
    )

    view_cols = [
        "Campionato", "Data", "Partita", "Pronostico", "Prob. pronostico %",
    ...
    coupon_df, coupon_summary = make_coupon(match_pred_df, coupon_mode, coupon_legs)
    ...
    coupon_df, coupon_summary = make_coupon_from_manual_odds(
        evaluated_odds, coupon_mode, coupon_legs
    )

    st.markdown(f"### 🎟️ Schedina suggerita — {coupon_mode}")
    if not coupon_df.empty:
        m1, m2, m3 = st.columns(3)
        m1.metric("Eventi", coupon_summary.get("N. eventi", 0))
        ...
        st.caption(
            "La schedina usa solo quote realmente inserite/precompilate. "
            "La probabilità combinata è un'approssimazione che moltiplica le probabilità dei singoli eventi."
        )
    else:
        st.info(
            "Nessuna combinazione soddisfa ancora i criteri della modalità scelta. "
            "Inserisci/correggi le quote sopra oppure prova una modalità diversa."
        )

    with st.expander("🧠 Dettagli forza squadre e giocatori"):
        detail = [
            "Campionato", "Partita", "Forma casa PPG", "Forma trasferta PPG",
