...
if test_real_player_odds:
    ...
    if real_odds_rows:
        ...
        # Ordine utile: tiri, SOT, cartellino; quota migliore in alto a parità di mercato/valore.
        odds_df = odds_df.sort_values(
            ["Campionato", "Partita", "Mercato API", "Valore API", "Quota"],
        ...
        preferred_odds_cols = [
            "Campionato", "Data partita", "Partita", "Bookmaker",
            "Mercato API", "Valore API", "Quota", "Prob. implicita %",
            "Bet ID", "Fixture ID", "Ultimo update",
        ]
        st.dataframe(
        ...
        best = (
            ...
            .drop_duplicates(subset=["Fixture ID", "Mercato API", "Valore API"], keep="first")
        )
        with st.expander("🏆 Migliore quota trovata per ogni linea"):
            best_cols = [
                "Campionato", "Partita", "Bookmaker", "Mercato API",
                "Valore API", "Quota", "Prob. implicita %",
            ]
            st.dataframe(
