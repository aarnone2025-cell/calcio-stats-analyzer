...
def get_upcoming_fixtures(league_id, season, days_ahead=14):
    ...
    for f in fixtures:
        ...
        dt_raw = f.get("fixture", {}).get("date")
        if not dt_raw or status not in {"NS", "TBD"}:
            continue
...
def opponent_market_context(df, stat, threshold, min_minutes, next_opponent, league_avg):
    """
...
for league_label, league_id in LEAGUES.items():
    ...
    try:
        ...
        future, future_errors = get_upcoming_fixtures(league_id, CURRENT_SEASON, 14)
        ...
        for f in future:
            ...
            when_label = "OGGI" if delta_days == 0 else ("DOMANI" if delta_days == 1 else fixture_dt.strftime("%d/%m"))
